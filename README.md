...
User passwords are **never stored in plain text**. During registration and password updates, the application hashes each password using **SHA-256** through the **Cryptox** utility before storing it in the SQLite database.

This provides:
- Secure password storage through one-way hashing
- Protection against accidental password disclosure from the database
- Password verification by hashing the entered password and comparing it with the stored hash during login

This mechanism is implemented in the authentication workflow using Cryptox's SHA-256 hashing functionality.
