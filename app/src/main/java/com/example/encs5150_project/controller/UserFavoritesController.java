...
public class UserFavoritesController {
    ...
    public UserFavoritesController(EventRepository eventRepository, UserRepository userRepository, FavouriteRepository favouriteRepository, SharedPrefManager sharedPrefManager) {
    ...
    public List<EventSummary> performSearch(String filterBy, boolean isAscending, String query) {
        ...
        List<Long> favoriteIds = favouriteRepository.getFavoriteEventIds(user.getId());
        List<EventSummary> rawAscending = eventRepository.searchEventSummariesForUser(filterBy, true, cleanQuery, user);
        ascendingList.clear();
        for (EventSummary summary : rawAscending) {
            if (favoriteIds.contains(summary.event().getId())) {
                ascendingList.add(summary);
            }
        }

        List<EventSummary> rawDescending = eventRepository.searchEventSummariesForUser(filterBy, false, cleanQuery, user);
        descendingList.clear();
        for (EventSummary summary : rawDescending) {
            if (favoriteIds.contains(summary.event().getId())) {
                descendingList.add(summary);
            }
        }
